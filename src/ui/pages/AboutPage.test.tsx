...
describe("AboutPage", () => {
  it("explains the public demo, MailHog, management access and PayPal sandbox", () => {
    ...
    expect(screen.getByText(/The database migration creates the root administrator/i)).toBeInTheDocument();
    expect(screen.getByText(/use Forgot password\? with the root email/i)).toBeInTheDocument();
    const credentials = screen.getByLabelText("Root administrator credentials");
